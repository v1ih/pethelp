...
export default function VeterinarianScheduleScreen() {
  ...
  return (
        ...
        <div className="mt-6 flex max-w-full items-center gap-1 overflow-x-auto rounded-full border border-border bg-card p-1 shadow-sm [-ms-overflow-style:none] [scrollbar-width:none] sm:w-max [&::-webkit-scrollbar]:hidden">
          <button
            type="button"
            onClick={() => setScheduleTab('calendar')}
            className={`inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full px-4 py-2 text-sm transition-colors ${scheduleTab === 'calendar' ? 'bg-primary text-white' : 'text-foreground hover:bg-muted'}`}
          >
          ...
          <button
            ...
            className={`inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full px-4 py-2 text-sm transition-colors ${scheduleTab === 'manage' ? 'bg-primary text-white' : 'text-foreground hover:bg-muted'}`}
          >
